...
**Integritas dan akses.** Siswa tidak boleh melewati checkpoint, LKPD, refleksi, atau prasyarat modul. Reader menggunakan `isCurrentModuleLocked` dengan kondisi bypass `user?.role !== 'admin'`. `AntiCopyPasteGuardian` memblokir paste, drop, `Ctrl/Cmd+V`, dan `Shift+Insert` pada jawaban pembelajaran. Pengecualian dibatasi oleh allowlist eksplisit `name`/`id` (pencocokan tidak peka huruf besar-kecil): identitas `studentName`, `studentNis`, `studentClass`, dan `submissionDate`, termasuk ID formulir generik `gen_studentName`, `gen_studentNis`, `gen_studentClass`, dan `gen_submissionDate`; serta field bukti `driveUrl`, `evidenceDriveUrl`, dan `gen_driveUrl`. Atribut seperti `data-allow-paste="true"` tidak memberi izin paste sendiri.
