...
**Materi interaktif formatif Modul 02–16.** `src/utils/orientasiInteractiveMaterials.ts` adalah katalog server-side untuk 15 slug kanonik Modul 02–16; setiap entri memiliki tepat dua aktivitas bertipe `explore`, `scenario`, `sequence`, atau `checklist`, dengan detail instruksional setiap butir yang selalu terlihat. Umpan balik diberikan secara lokal saat siswa berinteraksi; pada checklist respons disampaikan melalui live region. `src/components/modul/InteractiveModuleMaterial.astro` merender katalog tersebut sebagai kartu dark-slate berkontras tinggi dengan tombol native, state aksesibel, `data-*`, dan script Astro scoped. Urutan pada aktivitas `sequence` benar-benar dapat dipindah/diurutkan ulang; perilaku DOM lokalnya berada di `src/utils/interactiveModuleMaterialBehavior.ts` agar reorder, validasi, dan state pilihan dapat diuji secara terpisah. Reader memasang renderer hanya pada jalur Modul 02–16 dan mempertahankan Markdown sebagai bacaan rujukan; Modul 01 tetap memakai `InteractiveMaterialP1.astro`. Ini adalah interaksi formatif lokal yang ketat: tidak ada XP, API call, penyimpanan data/browser storage, perubahan state tab/gating, atau jalur bypass checkpoint. Checkpoint Quest tetap satu-satunya gerbang reward dan progres.
...
**Status rilis materi interaktif per 2026-08-08 (dikoreksi).** Deployment `dpl_CMVnY7i2RtM2SWyz299bzXssAkT2` READY di `https://orientasi-interactive-materials-5i6fvi902-agumelars-projects.vercel.app` dan alias `https://orientasi-interactive-materials.vercel.app` adalah proyek standalone, **bukan** project/domain production kanonik. Pemeriksaan sebelumnya terhadap `/pembelajaran` tanpa sesi mengikuti redirect login, sehingga tidak membuktikan isi katalog dan tidak boleh digunakan untuk mengklaim nama `pengenalan-html5-smk`, `dasar-basis-data-sql`, atau `konsep-oop-javascript` tidak ada di halaman terproteksi. Suite final Orientasi lulus 14/14 termasuk Happy DOM DOM regression untuk interaksi sequence. `happy-dom` 20.11.2 adalah dev-only dan dipatch; audit mencatat 0 critical, tetapi high chain yang tidak terkait (`@astrojs/vercel` → `@vercel/routing-utils` → `path-to-regexp` dan `sharp`) tetap menjadi tindak lanjut pemeliharaan dependency, bukan status audit global bersih. Deploy kanonik `dpl_A8KAA1B3dC9oZb4XPcrEvUUnDW2Z` di `https://agunggumelarsaputra-qdw4qvsy3-agumelars-projects.vercel.app` masih berstatus `UNKNOWN` setelah CLI timeout; `vercel inspect` hanya mencatat alias hostname otomatis Vercel, bukan `www.agunggumelarsaputra.com`. Karena itu belum ada rilis kanonik baru yang dapat diklaim. Smoke domain kanonik yang tercatat: `/` HTTP 200, `/pembelajaran` tanpa sesi HTTP 302 ke `/login?redirect=%2Fpembelajaran`, dan POST checkpoint tanpa sesi HTTP 401. Uji alur atau isi katalog terproteksi harus memakai akun siswa legitimate yang telah enrollment; jangan membuat akun palsu atau membypass autentikasi.
