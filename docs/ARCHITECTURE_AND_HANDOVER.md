...
**Sumber kebenaran.** `orientasi-pplg-01-pengantar-skill-passport` adalah acuan perilaku dan mutu untuk seluruh modul Orientasi PPLG. Keputusan ruang lingkup dan kontrak yang disetujui dicatat di `docs/superpowers/specs/2026-08-08-orientasi-pplg-module-one-parity-design.md`; keduanya wajib dibaca sebelum menambah atau mengubah Modul 02–16.
...
**Materi interaktif formatif Modul 02–16 dan urutan reader.** `src/utils/orientasiInteractiveMaterials.ts` adalah katalog server-side untuk 15 slug kanonik Modul 02–16; setiap entri memiliki `teacherMessage`, hero faktual, serta tepat dua scene bertipe `explore`, `scenario`, `sequence`, atau `checklist`. Reader wajib mempertahankan urutan sibling DOM berikut: (1) `details[data-reference-material]` berisi **Bacaan Rujukan & Materi Lengkap**, (2) `TeacherMessageCard` sebagai sibling yang terlihat tanpa membuka `details`, (3) `OrientasiLearningScene` yang merender hero dahulu kemudian dua scene, dan (4) area `InteractiveKnowledgeCheck` setelah semua scene. Modul 01 mempertahankan jalur baseline miliknya dan tidak memakai shell Modul 02–16.

`src/components/modul/OrientasiLearningScene.astro` merender kartu dark-slate berkontras tinggi dengan tombol native, state aksesibel, `data-*`, dan script Astro scoped. Umpan balik choice/scenario/checklist tinggal pada kartu scene terkait melalui live region; urutan pada scene `sequence` dapat dipindah naik/turun dan divalidasi. Perilaku DOM lokalnya tetap berada di `src/utils/interactiveModuleMaterialBehavior.ts` agar feedback, reorder, validasi, dan state pilihan dapat diuji terpisah. Seluruh scene bersifat formatif lokal yang ketat: tidak ada XP, API call, penyimpanan data/browser storage, perubahan state tab/gating, atau jalur bypass checkpoint. Checkpoint Quest tetap satu-satunya gerbang reward dan progres. Khusus Modul 02, konteks faktualnya adalah profesi PPLG dan handoff kerja tim produk; jangan menggantinya dengan contoh Skill Passport milik Modul 01.

**Prasyarat clean checkout reader.** `src/components/modul/SmartMarkdownWrapper.astro` wajib tetap tracked sebagai dependency reader. Ketiadaannya pernah menyebabkan clean checkout tidak dapat build; jangan menjadikannya untracked atau menggantikan kebutuhan ini dengan salinan file workspace manual.
...
**Urutan verifikasi wajib.** Jalankan focused guard `node --experimental-strip-types --test tests/orientasi-checkpoint-atomicity.test.ts tests/orientasi-submission-security.test.ts`, `npm run test:orientasi`, `npm run verify:orientasi-parity`, lalu `npm run build`. Suite Orientasi mencakup regression test executable Happy DOM untuk perilaku `sequence` (reorder, validasi, dan aksesibilitas state); guard parity memeriksa wiring endpoint/reader, 16 Markdown, katalog Quest/LKPD, posisi tombol selesai, dan kontrak gating. Setelah seluruhnya berhasil, deploy dengan `vercel --prod --yes`.
...
**Status rilis materi interaktif per 2026-08-08 (final).** Deployment `dpl_CMVnY7i2RtM2SWyz299bzXssAkT2` READY di `https://orientasi-interactive-materials-5i6fvi902-agumelars-projects.vercel.app` dan alias `https://orientasi-interactive-materials.vercel.app` adalah proyek standalone, **bukan** project/domain production kanonik. Pemeriksaan tanpa sesi terhadap `/pembelajaran` mengikuti redirect login, sehingga tidak membuktikan isi katalog dan tidak boleh digunakan untuk mengklaim nama `pengenalan-html5-smk`, `dasar-basis-data-sql`, atau `konsep-oop-javascript` tidak ada di halaman terproteksi. Suite final Orientasi lulus 14/14 termasuk Happy DOM DOM regression untuk interaksi sequence. `happy-dom` 20.11.2 adalah dev-only dan dipatch; audit mencatat 0 critical, tetapi high chain yang tidak terkait (`@astrojs/vercel` → `@vercel/routing-utils` → `path-to-regexp` dan `sharp`) tetap menjadi tindak lanjut pemeliharaan dependency, bukan status audit global bersih. Percobaan deploy kanonik `dpl_A8KAA1B3dC9oZb4XPcrEvUUnDW2Z` tetap `UNKNOWN` setelah CLI timeout dan bukan dasar klaim rilis. Deployment kanonik final `dpl_8RN2agHFQJmDZX2gLPZ6xxEMbrmL` READY di `https://agunggumelarsaputra-q3bm6qzbb-agumelars-projects.vercel.app`, teralias ke `https://www.agunggumelarsaputra.com` serta apex. Smoke final: `/` HTTP 200, `/pembelajaran` tanpa sesi HTTP 302 ke `/login?redirect=%2Fpembelajaran`, dan `POST /api/gamification/claim-checkpoint` tanpa sesi HTTP 401. Uji alur atau isi katalog terproteksi tetap harus memakai akun siswa legitimate yang telah enrollment; jangan membuat akun palsu atau membypass autentikasi.

**Status produksi dan uji siswa per 2026-08-08 (terkini).** Hambatan `TEAM_ACCESS_REQUIRED` sebelumnya berasal dari email author Git lokal yang salah: `agunggumelarsaputra@gmail.com`, bukan email GitHub pemilik Hobby `agumelarsaputra@gmail.com`. Commit metadata `fa5f653` menggunakan author yang benar dan deployment kanonis berikutnya `dpl_2VEPjQ6XzsgANCAJdwv2ZcgCaUJ1` sudah `READY` serta beralias `www`. Patch hardening form terbaru commit `7c04325` juga telah dideploy dari checkout bersih pada `dpl_Dvfrd3yHx6z8SQNmsK65Yd3WgzWa` (`READY`, `https://agunggumelarsaputra-kssax1kpq-agumelars-projects.vercel.app`, alias `https://www.agunggumelarsaputra.com`). Untuk akun Hobby, jangan mencoba menambah anggota team: Vercel mensyaratkan author commit berasal dari pemilik akun Hobby yang terhubung ke GitHub. Bila ganti mesin, set `git config user.name "agumelar"` dan `git config user.email "agumelarsaputra@gmail.com"` sebelum commit/deploy.

**Rekaman uji production sah.** Dengan izin eksplisit pemilik, akun siswa uji yang sudah enrollment menuntaskan checkpoint, LKPD, jurnal refleksi, dan completion Modul 01; isian berlabel `Uji rilis Codex 2026-08-08` sengaja tersimpan sebagai bukti rilis, lalu Modul 02 terbuka oleh policy server. Pemeriksaan UI Modul 02 mengonfirmasi urutan `details[data-reference-material]` → aside Pesan Guru Pengampu RPL di luar `details` → hero/dua scene → checkpoint; satu interaksi Frontend Developer hanya memberi feedback lokal. Jangan hapus atau ubah data uji ini tanpa instruksi pemilik.

**Hardening form hydration.** `src/utils/formHydrationGuard.ts` memasang guard pada `InteractiveLkpdP1` dan `InteractiveReflectionForm`: jika siswa sudah mengetik, memilih, atau menekan tombol di form, respons `GET /api/submissions/get` yang terlambat tidak boleh mengganti nilai mereka. `tests/orientasi-form-hydration.test.ts` adalah regression test perilaku; jalankan bersama `npm run test:orientasi`. Pada uji browser, automasi `fill()`/injeksi teks dapat diperlakukan sebagai *paste* oleh `AntiCopyPasteGuardian`; gunakan keystroke nyata untuk menguji jawaban belajar. Hal ini adalah proteksi integritas yang diharapkan, bukan kegagalan penyimpanan.

Production `www` yang tetap aktif adalah deployment lama `dpl_FgAixUKU1VJM6wUAP9v2BfRJzidu`. Smoke baseline lama pada 2026-08-08: `/` HTTP 200, `/pembelajaran` guest HTTP 302 ke `/login?redirect=%2Fpembelajaran`, dan POST `/api/gamification/claim-checkpoint` guest HTTP 401. Jangan gunakan hasil itu untuk mengklaim scene baru telah dirilis. Uji akun siswa sah sengaja ditunda karena deployment fitur belum `READY`; tidak ada login maupun mutasi submission, progress, atau XP akun uji. Setelah akses team diperbaiki, ulangi deploy dari commit yang direview, pastikan inspect `READY` dan alias mencakup `www`, baru jalankan Module 01 Quest → LKPD → refleksi → selesai dan inspeksi urutan/fakta/interaksi Module 02, lalu logout serta catat seluruh mutasi.
