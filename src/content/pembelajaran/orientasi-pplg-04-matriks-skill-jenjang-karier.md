---
title: "Matriks Skill, Kurikulum PPLG, & Jenjang Karier"
description: "Membedah Hard Skills vs Soft Skills, menghubungkan mata pelajaran Pengembangan Perangkat Lunak dan Gim (PPLG) Kelas 10-12 dengan industri, serta memahami tangga jenjang karier IT."
category: "Orientasi PPLG (OR-01)"
level: "Pemula"
order: 4
duration: "2 JP (90 Menit)"
tags: ["Matriks Skill", "Hard Skill", "Soft Skill", "Jenjang Karier", "Kurikulum PPLG", "OR-01"]
teacherTip: "Tekankan kepada siswa bahwa kemampuan teknis (coding) hanya membuka pintu wawancara, tetapi soft skill (komunikasi & integritas) yang menentukan keberlanjutan karier mereka di industri."
---

# Matriks Skill, Kurikulum PPLG, & Jenjang Karier

Banyak pemula mengira bahwa menjadi programmer yang sukses hanya butuh jago menghafal sintaks bahasa pemrograman. Faktanya, hasil riset industri IT global membuktikan bahwa **kombinasi seimbang antara Hard Skills (kemampuan teknis) dan Soft Skills (keterampilan interpersonal & karakter)** adalah kunci utama keberhasilan seorang pengembang software.

Modul ini akan memandu kalian memetakan keterampilan apa saja yang dibutuhkan industri, bagaimana kurikulum Pengembangan Perangkat Lunak dan Gim (PPLG) di SMKN 1 Rongga mempersiapkannya, serta tahapan tangga karier dari seorang *Junior Developer* hingga *Chief Technology Officer (CTO)*.

---

## 📌 Identitas Pembelajaran

| Komponen | Keterangan |
| :--- | :--- |
| **Mata Pelajaran** | Orientasi PPLG (Dasar-Dasar PPLG) |
| **Fase / Kelas / Semester** | Fase E / Kelas 10 RPL / Semester Ganjil |
| **Kode Skill Passport** | **OR-01** (Sprint 1 - Pertemuan 4 & 5) |
| **Elemen CP** | Wawasan Dunia Kerja Bidang Pengembangan Perangkat Lunak dan Gim |
| **Output / Evidence** | LKPD P4: Lembar Profil Profesi & Matriks Kebutuhan Skill (PDF) |

---

## 🎯 Tujuan Pembelajaran

1. **Membedakan Hard Skills & Soft Skills**: Siswa dapat mengkategorikan keterampilan teknis dan non-teknis yang esensial di bidang PPLG.
2. **Memetakan Mata Pelajaran PPLG**: Siswa memahami fungsi mata pelajaran di kelas 10, 11, dan 12 sebagai anak tangga menuju karier impian.
3. **Memahami Tangga Jenjang Karier IT**: Siswa mampu mengidentifikasi tanggung jawab dari level *Intern*, *Junior*, *Mid-level*, *Senior*, hingga *Lead Engineer*.
4. **Melakukan Analisis Kebutuhan Loker (Job Vacancy)**: Siswa mampu membaca dan membedah kualifikasi lowongan kerja nyata di industri.

---

## 📖 Materi Pembelajaran: Matriks Skill PPLG

```
                 ┌──────────────────────────────────────────────┐
                 │       ANATOMI SKILL REKAYASA PERANGKAT LUNAK  │
                 └──────────────────────┬───────────────────────┘
                                        │
             ┌──────────────────────────┴──────────────────────────┐
             ▼                                                     ▼
     [ HARD SKILLS (Teknis) ]                             [ SOFT SKILLS (Karakter) ]
     • Algoritma & Logika Pemrograman                     • Problem Solving & Kritis
     • HTML, CSS, JavaScript, PHP, Python                 • Komunikasi & Kolaborasi Tim
     • Desain Basis Data (SQL/NoSQL)                      • Adaptabilitas & Belajar Cepat
     • Version Control (Git & GitHub)                     • Manajemen Waktu & Integritas
     • Pengujian Software & Debugging                     • Bahasa Inggris Teknis
```

---

### 1. Keterkaitan Mata Pelajaran PPLG SMKN 1 Rongga dengan Industri

Kurikulum Pengembangan Perangkat Lunak dan Gim (PPLG) dirancang bertahap agar kalian memiliki pondasi yang kuat:

| Tingkat | Mata Pelajaran / Konsentrasi | Output Keterampilan yang Dibangun |
| :--- | :--- | :--- |
| **Kelas 10 (Fase E)** | • **Orientasi PPLG**<br>• **Dasar Pemrograman (Algoritma & Logika)**<br>• **Pemodelan Perangkat Lunak** | Memahami ekosistem industri, logika algoritma dasar, flowchart, dan dasar struktur data. |
| **Kelas 11 (Fase F)** | • **Pemrograman Web (Frontend & Backend)**<br>• **Basis Data (RDBMS & SQL)**<br>• **Pemrograman Berorientasi Objek (PBO)** | Mampu membangun web aplikasi dinamis, API terhubung database, dan sistem CRUD modular. |
| **Kelas 12 (Fase F)** | • **Pemrograman Mobile (Flutter / Android Native)**<br>• **Produk Kreatif & Kewirausahaan (PKK)**<br>• **Praktik Kerja Lapangan (PKL Industri)** | Membangun aplikasi smartphone, proyek portofolio tim, serta pengalaman magang nyata di industri. |

---

### 2. Tangga Jenjang Karier di Industri Perangkat Lunak

```
  [ Level 5 ]  CTO / VP of Engineering / Software Architect
  ▲            (Menentukan arah teknologi perusahaan & arsitektur sistem skala besar)
  │
  [ Level 4 ]  Tech Lead / Engineering Manager
  ▲            (Memimpin tim developer, code review, dan manajemen sprint proyek)
  │
  [ Level 3 ]  Senior Developer (Pengalaman 5+ Tahun)
  ▲            (Merancang arsitektur kode kompleks, optimasi performa & keamanan)
  │
  [ Level 2 ]  Mid-Level Developer (Pengalaman 2 - 4 Tahun)
  ▲            (Bekerja mandiri menyelesaikan fitur dari awal hingga siap produksi)
  │
  [ Level 1 ]  Junior Developer / Intern (Lulusan Baru / Siswa PKL)
               (Mengerjakan tugas coding spesifik di bawah bimbingan developer senior)
```

---

### 3. Simulasi Job Fair Kelas PPLG

Pada pertemuan ini, kalian akan melakukan simulasi **Job Fair Kelas**:
- Sebagian siswa berperan sebagai **Tim HRD / Tech Recruiter** yang membuka *booth* lowongan profesi (misal: "Lowongan Junior Frontend Developer di Startup FinTech").
- Siswa lainnya berperan sebagai **Pelamar Kerja (Job Seeker)** yang berkeliling membawa portofolio Skill Passport untuk mencari tahu kualifikasi yang dicari oleh industri.

---

---

## ⚡ Langkah Selanjutnya: Aktivitas Belajar

Gunakan tab di bagian atas modul ini untuk menyelesaikan tahapan belajar kalian:

1. 📖 **Tab 1 (Materi & Checkpoint)**: Pelajari materi lalu tuntaskan kuis checkpoint tiga tahap agar Tab 2 terbuka.
2. 📝 **Tab 2 (Form LKPD Interaktif)**: Isi dan kirim LKPD secara mandiri; sertakan URL Google Drive portofolio bila diperlukan agar Tab 3 terbuka (+25 XP).
3. 💭 **Tab 3 (Jurnal Refleksi)**: Tulis dan kirim refleksi mandiri (+15 XP), kemudian klik **“Tandai Selesai & Buka Modul Selanjutnya”** untuk menuntaskan modul.
4. 🎯 **Tab 4 (Panduan Kriteria Guru)**: Gunakan matriks KKTP untuk memeriksa target karya kalian.

---

## 💡 Catatan Guru Pengampu

> *"Semua Senior Developer pernah menjadi Junior yang bingung saat pertama kali melihat kode program. Yang membedakan mereka yang berhasil adalah daya tahan untuk tidak mudah menyerah saat menghadapi pesan error!"*
> 
> — **Agung Gumelar Saputra, S.Tr.T, Gr**
